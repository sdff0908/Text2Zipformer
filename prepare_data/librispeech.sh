...
# corpus dir for text transcript
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  ...
  if [ ! -e data/fbank/.librispeech.done ]; then
    python3 preprocess/compute_fbank_librispeech.py
  ...
  if [ ! -e data/fbank/.librispeech-validated.done ]; then
    ...
    for part in ${parts[@]}; do
      python3 preprocess/validate_manifest.py \
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  ...
  if [ ! -f $lang_dir/transcript_words.txt ]; then
    ...
    tr '[:upper:]' '[:lower:]' < $lang_dir/transcript_words.txt > $lang_dir/lowered_transcript_words.txt
  fi
fi